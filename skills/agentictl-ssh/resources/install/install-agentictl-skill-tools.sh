...
if [ -z "${BASH_VERSION:-}" ]; then
  ...
  printf '%s\n' "install-agentictl-skill-tools.sh requires bash; run: bash $0 ..." >&2
  exit 2
fi
set -euo pipefail
IFS=$'\n\t'
...
install -m 0755 "$SKILL_DIR/scripts/agentictl-fleet-sync.sh" "$BIN_DIR/agentictl-fleet-sync.sh"
...
printf '{"ok":true,"bin_dir":%s,"share_dir":%s,"installed":["agentictl-nodes","agentictl-approval-tool.sh","agentictl-fleet-sync.sh","agentictl-node-tool.sh","agentictl-ssh-tool.sh","agentictl-bootstrap-instructions.sh","agentictl-node-upgrade.sh","resources/dist/agentictl-0.1.0.tar.gz","resources/node/install-node.sh"]}\n' \
  "$(json_string "$BIN_DIR")" \

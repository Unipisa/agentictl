...
Options:
  ...
  --user NAME                       Dedicated system user, default: agentictl.
...
install -d -m 0750 -o "$USER_NAME" -g "$USER_GROUP" "$BASE_DIR/state" "$BASE_DIR/state/backups" "$BASE_DIR/state/incoming"
...
auth_tmp="$(mktemp)"
if [[ -f "$auth_file" ]]; then
  grep -v 'agentictl-managed-' "$auth_file" > "$auth_tmp" || true
fi
build_key_line readonly "$READONLY_PUBLIC_KEY_FILE" >> "$auth_tmp"
if [[ -n "$ACTION_PUBLIC_KEY_FILE" ]]; then
  build_key_line act "$ACTION_PUBLIC_KEY_FILE" >> "$auth_tmp"
fi
install -m 0600 -o "$USER_NAME" -g "$USER_GROUP" "$auth_tmp" "$auth_file"
rm -f "$auth_tmp"

if command -v sudo >/dev/null 2>&1; then
  sudoers_tmp="$(mktemp)"
  printf '%s ALL=(root) NOPASSWD: %s/bin/agentictl-act *\n' "$USER_NAME" "$BASE_DIR" > "$sudoers_tmp"
  if command -v visudo >/dev/null 2>&1; then
    visudo -cf "$sudoers_tmp" >/dev/null
  fi
  install -m 0440 -o root -g root "$sudoers_tmp" "/etc/sudoers.d/agentictl"
  rm -f "$sudoers_tmp"
else

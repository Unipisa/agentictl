...
if [ -z "${BASH_VERSION:-}" ]; then
  ...
  printf '%s\n' "install-node.sh requires bash; run: bash $0 ..." >&2
  exit 2
fi
set -euo pipefail
IFS=$'\n\t'
...
ALLOW_PACKAGE_UPGRADE="nvtop htop jq"
...
Options:
  ...
  --allow-package-upgrade LIST      Space-separated package upgrade allowlist.
  --allow-package-upgrade-all BOOL  Allow full package-manager upgrade, default: false.
...
if [[ ! -e "$BASE_DIR/config/policy.env" ]]; then
  tmp_policy="$(mktemp)"
  {
    ...
    printf 'ALLOW_PACKAGE_UPGRADE=%q\n' "$ALLOW_PACKAGE_UPGRADE"
    printf 'ALLOW_PACKAGE_UPGRADE_ALL=%q\n' "$ALLOW_PACKAGE_UPGRADE_ALL"
  ...
  ensure_policy_line ALLOW_PACKAGE_UPGRADE "$ALLOW_PACKAGE_UPGRADE"
  ensure_policy_line ALLOW_PACKAGE_UPGRADE_ALL "$ALLOW_PACKAGE_UPGRADE_ALL"
fi
